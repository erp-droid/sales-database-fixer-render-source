...
export default async function TvAccountDetailPage({ params }: TvAccountDetailPageProps) {
  ...
  await requireTvAccess(`/tv/accounts/${encodeURIComponent(id)}`);
      ...
      <div className={styles.shell}>
        <header className={styles.topbar}>
          <div className={styles.titleBlock}>
            ...
            <h1 className={styles.title}>{primary?.companyName ?? "Account"}</h1>
            <p className={styles.subtitle}>{primary?.address || "No address"}</p>
          </div>
          <nav className={styles.nav} aria-label="TV navigation">
            ...
            <Link href={`/accounts?search=${encodeURIComponent(primary?.companyName ?? "")}`}>Full app</Link>
          </nav>
        </header>

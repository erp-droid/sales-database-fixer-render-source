...
type WorkbookRow = {
  ...
  contactClass: ContactClassKey | null;
};
...
const DEFAULT_REPORT_PATH = path.join(process.cwd(), "data", "glendale-data-import-report.json");
const DEFAULT_WORKBOOK_PATH = path.join(
  process.cwd(),
  "Glendale Group Database 260325 (1).xlsx",
);
const DEFAULT_SALES_REP_ID = "109382";
const DEFAULT_SALES_REP_NAME = "Travis Justin Rumney";
const WORKBOOK_ROW_OVERRIDES: Record<number, Partial<WorkbookRow>> = {
  165: {
    postalCode: "L2V 4Y6",
  },
  403: {
  ...
  496: {
    addressLine1: "1905 Aston Ave",
    addressLine2: "Suite 100",
    city: "Carlsbad",
    state: "CA",
    postalCode: "92008",
    country: "US",
    accountPhone: "858 675 1100",
  },
  510: {
    city: "Mississauga",
    postalCode: "L4W 5A1",
...
function buildAddressKeyWithoutCityAndPostal(
  parts: Parameters<typeof buildAddressKey>[0],
): string {
  return [
    normalizeAddressComparable(parts.addressLine1),
    normalizeAddressComparable(parts.addressLine2),
...
function buildStrictAccountKeyNoCityPostal(
  companyName: string,
  address: Parameters<typeof buildAddressKey>[0],
): string {
  return `${normalizeComparable(companyName)}|${buildAddressKeyWithoutCityAndPostal(address)}`;
}

function buildStrippedAccountKeyNoCityPostal(
  companyName: string,
  address: Parameters<typeof buildAddressKey>[0],
): string {
  return `${stripCompanySuffixes(companyName)}|${buildAddressKeyWithoutCityAndPostal(address)}`;
}
...
function indexExistingAccounts(groups: ExistingAccountGroup[]) {
  ...
  const exactNoCityPostal = new Map<string, ExistingAccountGroup[]>();
  const strippedNoCityPostal = new Map<string, ExistingAccountGroup[]>();
  ...
  for (const group of groups) {
    ...
    const keys = [
      ...
      [
        exactNoCityPostal,
        buildStrictAccountKeyNoCityPostal(group.representativeRow.companyName, address),
      ],
      [
        strippedNoCityPostal,
        buildStrippedAccountKeyNoCityPostal(group.representativeRow.companyName, address),
      ],
    ] as const;
...
function matchExistingAccount(
  ...
  const attempts = [
    ...
    {
      matchType: "exact-name-address-no-city-postal",
      candidates:
        indexes.exactNoCityPostal.get(
          buildStrictAccountKeyNoCityPostal(source.companyName, address),
        ) ?? [],
    },
    {
      matchType: "stripped-name-address-no-city-postal",
      candidates:
        indexes.strippedNoCityPostal.get(
          buildStrippedAccountKeyNoCityPostal(source.companyName, address),
        ) ?? [],
    },
  ];
...
function needsAccountUpdate(
  _existing: BusinessAccountRow,
  _source: StagedAccount,
  _options?: {
    salesRepId?: string | null;
  },
): string[] {
...
function parseArgs(argv: string[]): ImportOptions {
  ...
  let salesRepId: string | null = DEFAULT_SALES_REP_ID;
  let salesRepName: string | null = DEFAULT_SALES_REP_NAME;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      process.stdout.write(
        [
          "Usage:",
          "  npx tsx scripts/import-glendale-data.ts [--apply] [--limit N] [--workbook path] [--report-file path] [--sales-rep-id 109382] [--sales-rep-name 'Travis Justin Rumney']",
          "",
          "Behavior:",
          "  - dry-run by default",
          "  - with --apply, creates missing Glendale accounts, adds missing contacts to matched accounts, and assigns Travis as owner for newly created accounts",
...
function sanitizeWorkbookPhoneValue(value: string | null | undefined): string | null {
  const trimmed = trimToNull(value);
  if (!trimmed) {
    return null;
  }

  if (trimmed.includes("#ERROR!") || trimmed.includes("‚Ä") || trimmed.includes("�")) {
    return null;
  }
...
function extractPreferredWorkbookEmail(value: string | null | undefined): string | null {
  const trimmed = trimToNull(value);
  if (!trimmed) {
    return null;
  }
  ...
  const preferredInfoEmail = emails.find((email) => email.toLowerCase().startsWith("info@"));
  return normalizeEmail(preferredInfoEmail ?? emails[0] ?? null);
}

function looksLikeCanadianPostalCode(value: string | null | undefined): boolean {
  const normalized = normalizePostalCode(value ?? "").replace(/\s+/g, "").toUpperCase();
  return /^[A-Z]\d[A-Z]\d[A-Z]\d$/.test(normalized);
}

function applyWorkbookRowOverrides(row: WorkbookRow): WorkbookRow {
  const override = WORKBOOK_ROW_OVERRIDES[row.rowNumber];
  if (!override) {
    return row;
  }
...
for row_number, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
    ...
    rows.append({
        ...
        "companyName": norm(row[0]) or "",
        "accountType": norm(row[1]),
        "address": norm(row[2]),
        "accountPhone": norm(row[3]),
        "unitNumber": norm(row[4]),
        "city": norm(row[5]),
        "postalCode": norm(row[6]),
        "industryType": norm(row[7]),
        "week": norm(row[8]),
        "contactFirstName": norm(row[9]),
        "contactLastName": norm(row[10]),
        "contactJobTitle": norm(row[11]),
        "contactEmail": norm(row[12]),
        "contactPhone": norm(row[13]),
        "contactClass": norm(row[14]),
    })
...
`;
  ...
  return parsed.map((row) => {
    ...
    let postalCode = normalizePostalCode(row.postalCode ?? "");
    if (looksLikeCanadianPostalCode(city) && !looksLikeCanadianPostalCode(postalCode)) {
      const swappedCity = normalizeCity(row.postalCode ?? "");
      city = swappedCity;
      postalCode = normalizePostalCode(row.city ?? "");
    }

    const normalizedCountry = /^\d{5}(?:-\d{4})?$/.test(postalCode.replace(/\s+/g, ""))
      ? "US"
      : "CA";
    const normalizedRow: WorkbookRow = {
      rowNumber: row.rowNumber,
      companyName: row.companyName,
      classId: normalizeBusinessAccountClass(row.accountType),
      addressLine1: collapseWhitespace(row.address ?? ""),
      addressLine2: normalizeAddressLine2(row.unitNumber),
      city,
      state:
        normalizedCountry === "US"
          ? ""
          : inferProvinceFromPostalCode(postalCode),
      postalCode:
        normalizedCountry === "CA" && !looksLikeCanadianPostalCode(postalCode)
          ? ""
          : postalCode,
      country: normalizedCountry,
      accountPhone: sanitizeWorkbookPhoneValue(row.accountPhone),
      industryType: trimToNull(row.industryType),
      week: canonicalWeek(row.week),
      contactFirstName: trimToNull(row.contactFirstName),
      contactLastName: trimToNull(row.contactLastName),
      contactJobTitle: trimToNull(row.contactJobTitle),
      contactEmail: extractPreferredWorkbookEmail(row.contactEmail),
      contactPhone: sanitizeWorkbookPhoneValue(row.contactPhone),
      contactClass: normalizeContactClass(row.contactClass),
    };

    return applyWorkbookRowOverrides(normalizedRow);
  });
}
...
function normalizeBusinessAccountClass(value: string | null | undefined): BusinessAccountClassCode {
  const normalized = collapseWhitespace(value).toLowerCase();
  return normalized === "customer" ? "CUSTOMER" : "LEAD";
}

function normalizeContactClass(value: string | null | undefined): ContactClassKey | null {
  const normalized = collapseWhitespace(value).toLowerCase();
  if (!normalized) {
    return null;
  }

  const mapping: Record<string, ContactClassKey> = {
    billing: "billing",
...
function normalizeAddressLine2(value: string | null | undefined): string {
  const trimmed = collapseWhitespace(value);
  if (!trimmed) {
    return "";
  }

  if (/^(suite|ste|unit|#)/i.test(trimmed)) {
    return trimmed;
  }

  return /^\d+$/.test(trimmed) ? `Unit ${trimmed}` : trimmed;
}

function inferProvinceFromPostalCode(value: string | null | undefined): string {
  const normalized = normalizePostalCode(value ?? "").replace(/\s+/g, "").toUpperCase();
  ...
  return mapping[first] ?? "ON";
}

function combineAddressComparable(addressLine1: string, addressLine2: string): string {
  return normalizeAddressComparable([addressLine1, addressLine2].filter(Boolean).join(" "));
}

function chooseMoreCompleteString(current: string, next: string): string {
  if (!current) {
    return next;
  }
  if (!next) {
  ...
  const currentComparable = normalizeComparable(current);
  const nextComparable = normalizeComparable(next);
...
function chooseMoreCompleteNullable(current: string | null, next: string | null): string | null {
  if (!current) {
    return next;
  }
  if (!next) {
...
function shouldMergeStagedAccount(candidate: {
  companyName: string;
  addressLine1: string;
  addressLine2: string;
  city: string;
  postalCode: string;
}, existing: StagedAccount): boolean {
  if (normalizeComparable(candidate.companyName) !== normalizeComparable(existing.companyName)) {
    return false;
  }

  const candidateCombinedAddress = combineAddressComparable(
    candidate.addressLine1,
    candidate.addressLine2,
  );
  const existingCombinedAddress = combineAddressComparable(
    existing.addressLine1,
    existing.addressLine2,
  );

  if (!candidateCombinedAddress || !existingCombinedAddress) {
    return false;
  }
  ...
  const existingCity = normalizeComparable(existing.city);
  if (candidateCity && existingCity && candidateCity !== existingCity) {
    return false;
  }

  const candidatePostal = normalizeComparable(candidate.postalCode);
  const existingPostal = normalizeComparable(existing.postalCode);
  if (candidatePostal && existingPostal && candidatePostal !== existingPostal) {
    return false;
  }
...
function buildContactDisplayName(firstName: string | null, lastName: string | null): string | null {
  return collapseWhitespace([firstName ?? "", lastName ?? ""].filter(Boolean).join(" ")) || null;
}

function buildStagedAccounts(rows: WorkbookRow[]): StagedAccount[] {
  const stagedAccounts: StagedAccount[] = [];

  for (const row of rows) {
    const companyName = collapseWhitespace(row.companyName);
    const addressLine1 = collapseWhitespace(row.addressLine1);
    const addressLine2 = collapseWhitespace(row.addressLine2);
    const city = normalizeCity(row.city);
    const state = normalizeState(row.state || inferProvinceFromPostalCode(row.postalCode));
    const postalCode = normalizePostalCode(row.postalCode);
    const country = collapseWhitespace(row.country || "CA").toUpperCase() || "CA";
    const industryType = collapseWhitespace(row.industryType ?? "");
    const accountPhone = normalizePhoneForSave(row.accountPhone);

    if (!companyName || !addressLine1) {
      continue;
    }

    let account = stagedAccounts.find((existing) =>
      shouldMergeStagedAccount(
        {
          companyName,
          addressLine1,
          addressLine2,
    ...
    if (!account) {
      ...
      if (account.classId !== row.classId) {
        account.conflicts.push(
          `Account type conflict on row ${row.rowNumber}: kept ${account.classId}.`,
        );
        if (account.classId !== "CUSTOMER" && row.classId === "CUSTOMER") {
          account.classId = "CUSTOMER";
        }
      }

      if (account.week !== row.week) {
        const nextWeek = betterWeek(account.week, row.week);
        if (account.week && row.week && account.week !== row.week) {
          account.conflicts.push(`Week conflict on row ${row.rowNumber}: kept ${nextWeek}.`);
        }
        account.week = nextWeek;
      }
    }

    account.companyName = chooseMoreCompleteString(account.companyName, companyName);
    account.addressLine1 = chooseMoreCompleteString(account.addressLine1, addressLine1);
    account.addressLine2 = chooseMoreCompleteString(account.addressLine2, addressLine2);
    account.city = chooseMoreCompleteString(account.city, city);
    account.state = chooseMoreCompleteString(account.state, state);
    account.postalCode = chooseMoreCompleteString(account.postalCode, postalCode);
    account.country = chooseMoreCompleteString(account.country, country);
    account.industryType = chooseMoreCompleteString(account.industryType, industryType);
    account.accountPhone = chooseMoreCompleteNullable(account.accountPhone, accountPhone);
    account.sourceRows.push(row.rowNumber);

    const displayName = buildContactDisplayName(row.contactFirstName, row.contactLastName);
    if (!displayName) {
      continue;
    }

    const parsedPhone = parsePhoneValue(row.contactPhone);
    upsertStagedContact(account, {
      displayName,
      email: normalizeEmail(row.contactEmail),
      phone: parsedPhone.phone,
      extension: parsedPhone.extension,
      jobTitle: trimToNull(row.contactJobTitle) ?? "",
      contactClass: row.contactClass ?? "sales",
  ...
  return stagedAccounts
    .map((account) => {
      ...
      account.sourceKey = buildStrictAccountKey(account.companyName, {
    ...
    .sort((left, right) => left.companyName.localeCompare(right.companyName, undefined, {
      sensitivity: "base",
    }));
}
  ...
  try {
    ...
    for (const { account, match } of stagedMatches) {
      ...
      try {
        ...
        let createdClass: BusinessAccountClassCode = account.classId;
        ...
        try {
          createdRawAccount = await createBusinessAccount(
            ...
            buildBusinessAccountCreatePayload({
              ...
              state: account.state,
              postalCode: account.postalCode,
              country: account.country as "CA",
            }),
            authCookieRefresh,
          );
        } catch (error) {
          const message = getErrorMessage(error).toLowerCase();
          if (createdClass === "CUSTOMER" && message.includes("customer management preferences form")) {
            createdClass = "LEAD";
            createdRawAccount = await createBusinessAccount(
              cookieValue,
              buildBusinessAccountCreatePayload({
